...
import { sampleWithRequiredData } from '../indicator.test-samples';
...
describe('Indicator Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.indicators?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to indicatorService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getIndicatorIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getIndicatorIdentifier).toHaveBeenCalledWith(entity);
