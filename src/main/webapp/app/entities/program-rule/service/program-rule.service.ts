...
export class ProgramRuleService {
  ...
  history(id: any): Observable<EntityArrayResponseType> {
    return this.http.get<IProgramRule[]>(`${this.resourceUrl}/${id}/audit`, { observe: 'response' });
  }

  compare(id: string, rev1: number, rev2: number): Observable<EntityArrayResponseType> {
    return this.http.get<IProgramRule[]>(`${this.resourceUrl}/${id}/compare/${rev1}/${rev2}`, { observe: 'response' });
  }
