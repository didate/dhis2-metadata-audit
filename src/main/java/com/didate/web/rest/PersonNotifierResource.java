 ...
 */
@RestController
@RequestMapping("/api/person-notifiers")
public class PersonNotifierResource {

    private static final Logger log = LoggerFactory.getLogger(PersonNotifierResource.class);
    ...
    public ResponseEntity<PersonNotifier> createPersonNotifier(@Valid @RequestBody PersonNotifier personNotifier)
        ...
        personNotifier = personNotifierService.save(personNotifier);
        return ResponseEntity.created(new URI("/api/person-notifiers/" + personNotifier.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, personNotifier.getId().toString()))
            .body(personNotifier);
    }
    ...
    public ResponseEntity<PersonNotifier> updatePersonNotifier(
        ...
        personNotifier = personNotifierService.update(personNotifier);
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<PersonNotifier>> getAllPersonNotifiers(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<PersonNotifier> getPersonNotifier(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deletePersonNotifier(@PathVariable("id") Long id) {
